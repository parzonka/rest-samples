...
if (phantom.args.length === 0) {
  ...
  console.log("");
  console
      .log("NOTE: This script depends on jasmine.TrivialReporter being used\non the page, for the DOM elements it creates.\n");
  phantom.exit(2);
} else {
  var args = phantom.args;
  var fs = require("fs"), pages = [], page, address, resultsKey, i, l;
  ...
  for (i = 0, l = args.length; i < l; i++) {
    ...
    // When initialized, inject the reporting functions before the page is
    // loaded
...
/**
 * Logs a message. Does not add a line-break for single characters '.' and 'F'
 * or lines ending in ' ...'
 *
 * @param msg
 */
function logAndWorkAroundDefaultLineBreaking(msg) {
  ...
  if (navigator.userAgent.indexOf("Windows") < 0
    ...
    var fs = require('fs');
    // system.stdout.write(msg) ? wait for
    // http://code.google.com/p/phantomjs/issues/detail?id=243 to be
    // implemented
 ...
 *
 * @param {function}
 *          fn The function to replace occurrences within.
 * @param {object}
 *          replacements Key => Value object of string replacements.
 */
 ...
 *          page The WebPage object to overload
 */
 ...
 *
 * @param {phantomjs.WebPage}
 *          page The WebPage object to inject functions into.
 * @param {string}
 *          key The name of the global object in which file data should be
 ...
 *
 * @param {phantomjs.WebPage}
 *          page The WebPage object to retrieve data from.
 * @param {string}
 *          key The name of the global object to be returned. Should be the same
 *          key provided to setupWriteFileFunction.
 */
 ...
 *
 * @param {string}
 *          status The status from opening the page via WebPage#open.
 * @param {phantomjs.WebPage}
 *          page The WebPage to be processed.
 */
      ...
      return page
          .evaluate(function() {
            require([ "jasmine" ], function(jasmine) {
              // if there's a JUnitXmlReporter, return a boolean indicating if
              // it is finished
              if (jasmine.JUnitXmlReporter) {
                return jasmine.JUnitXmlReporter.finished_at !== null;
              }
            });
            // otherwise, see if there is anything in a "finished-at"
            // element
    ...
    var ival = setInterval(
        function() {
          if (isFinished()) {
            ...
            var fs = require("fs"), xml_results = getXmlResults(page,
                resultsKey), output;
            for ( var filename in xml_results) {
              if (xml_results.hasOwnProperty(filename)
