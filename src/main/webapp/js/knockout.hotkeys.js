      ...
      ko.utils.arrayForEach(hotkeyHandlerShortcuts, function(eventName) {
        ko.bindingHandlers["hot" + eventName] = {
          'init' : function(element, valueAccessor, allBindingsAccessor,
            ...
            return ko.bindingHandlers['_hotkey']['init'].call(this, element,
                newValueAccessor, allBindingsAccessor, viewModel);
          }
        }
      });
            ...
            (function() {
              ...
              for ( var idxOutsideClosure = 0; idxOutsideClosure < valLength; idxOutsideClosure++) {
                (function() {
                  ...
                  // FIXME: doesn't work with two "+"
                  hotkey = hotkey.toLowerCase().replace(/\+/g, "");

                  ko.utils
                      .registerEventHandler(
                          ko.hotkeys.defaultOptions.bindElement,
                          eventName,
                          function(event) {

                            // Keypress represents characters, not special keys
                            var special = event.type !== "keypress"
                                && ko.hotkeys.keymap.specialKeys[event.which], character = String
                            ...
                            // TODO: Need to make sure this works consistently
                            // across platforms
                            ...
                            if (special) {
                              ...
                              possible[modif
                                  + ko.hotkeys.keymap.shiftNums[character]] = true;

                              // "$" can be triggered as "Shift+4" or "Shift+$"
                              // or just "$"
                              if (modif === "shift") {
                            ...
                            // FIXME - fixed: added visible check here...
                            if (possible[hotkey] && $(element).is(":visible")) {
                              $(event.target).change(); // FIXME - fixed: fire
                                                        // change event to
                                                        // populate model
                              ...
                              try {
                                if (!("if" in innerVal)
                                    || ko.utils
                                        .unwrapObservable(innerVal["if"])) // Only
                                                                            // apply
                                                                            // the
                                                                            // handler
                                                                            // function
                                ...
                                if (handlerReturnValue !== true) { // Normally
                                                                    // we want
                                                                    // to
                                                                    // prevent
                                                                    // default
